...
import static io.flutter.plugins.firebase.messaging.FlutterFirebaseMessagingUtils.CALL_STATE_ACCEPTED;
...
public class FlutterFirebaseMessagingPlugin extends BroadcastReceiver
  ...
  private String TAG="FirebaseMsgPlugin";
  ...
  public boolean onNewIntent(Intent intent) {
    ...
    if (intent == null || intent.getExtras() == null) {
      ...
      Log.e(TAG,"onNewIntent have intent");
    // Remote Message ID can be either one of the following...
    String messageId = intent.getExtras().getString("google.message_id");
    if (messageId == null) {
    ...
    if (messageId == null) {
      ...
      Log.e(TAG,"onNewIntent have message id"+messageId);

    RemoteMessage remoteMessage = FlutterFirebaseMessagingReceiver.notifications.get(messageId);

    // If we can't find a copy of the remote message in memory then check from our persisted store.
    if (remoteMessage == null) {
        Log.e(TAG,"onNewIntent remoteMessage null 1");
    ...
    if (remoteMessage == null) {
        Log.e(TAG,"onNewIntent remoteMessage null 2");
      return false;
    }
      Log.e(TAG,"onNewIntent have remoteMessage");
    ...
    Log.e(TAG,"onNewIntent intent.action kasdklajsdk "+intent.getAction());
      Log.e(TAG,"onNewIntent intent.action  kasdklajsdk"+intent.getAction().equals(CALL_STATE_ACCEPTED));
      if(intent.getAction().equals(CALL_STATE_ACCEPTED))
      {
          Log.e(TAG,"onNewIntent CALL_STATE_ACCEPTED 1");
          channel.invokeMethod(
                  "Messaging#onMessageOpenedAppWhenCallAccept",
                  FlutterFirebaseMessagingUtils.remoteMessageToMap(remoteMessage));
      }
      else
      {
          Log.e(TAG,"onNewIntent CALL_STATE_ACCEPTED 2");
